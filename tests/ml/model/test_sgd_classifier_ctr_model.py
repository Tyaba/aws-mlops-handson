...
def test_grid_search():
    ...
    dataset = Dataset(
        train=FeatureTarget(X_train, y_train),
        valid=FeatureTarget(X_valid, y_valid),
        test=None,
    )
    ...
    assert np.isin(best_alpha, [100, 1e-6, 5e-6, 1e-5, 5e-5])

    best_model = SGDClassifier(
        loss="log_loss", penalty="l2", random_state=42, alpha=best_alpha
    )
    best_model.fit(X_train, y_train)
    best_pred_probas = best_model.predict_proba(X_valid)[:, 1]
    best_score = roc_auc_score(y_valid, best_pred_probas)
    ...
    for alpha in [100, 1e-6, 5e-6, 1e-5, 5e-5]:
        if alpha != best_alpha:
            ...
            score = roc_auc_score(y_valid, pred_probas)
